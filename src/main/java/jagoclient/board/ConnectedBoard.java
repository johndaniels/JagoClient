...
public class ConnectedBoard extends Board
{
	...
	{	if (gameViewerState.getGameTree().current().content().main() && CGF.wantsmove()) return;
	...
	*/
	...
	{	if (gameViewerState.getGameTree().hasChildren()) return;
		...
		else gameViewerState.getGameTree().set(i,j); // try to set a new move
	}
	...
	*/
	...
	{	if (gameViewerState.getGameTree().hasChildren()) return;
		...
		if (CGF.wantsmove() && gameViewerState.getGameTree().current().content().main())
		{	CGF.moveset(i0,j0);
		}
		//gameViewerState.getGameTree().removegroup(i0,j0);
	}
	...
	*/
	...
	{	if (gameViewerState.getGameTree().current().content().main()
		...
		{	if (!gameViewerState.getGameTree().hasChildren())
			{	if (gameViewerState.getUiMode() != GameViewerState.UIMode.PLAY_BLACK && gameViewerState.getUiMode() != GameViewerState.UIMode.PLAY_WHITE) gameViewerState.getGameTree().clearremovals();
				CGF.undo();
			}
			return;
		}
	...
	*/
	...
	{	if (gameViewerState.getGameTree().hasChildren()) return;
		if (gameViewerState.getGameTree().current().content().main()) return;
		if (gameViewerState.getGameTree().current().content().main() && CGF.wantsmove())
		{	CGF.movepass(); return;
		}
	}
	...
	*/
	...
	{	if (!gameViewerState.getGameTree().hasChildren() && gameViewerState.getGameTree().current().content().main() && CGF.wantsmove()) return;
	}
